...
},{"ohauth":6,"store":14,"xtend":15}],6:[function(require,module,exports){
'use strict';
...
},{"jshashes":7,"xtend":9}],7:[function(require,module,exports){
(function (global){
/**
...
},{"./has-keys":8,"object-keys":10}],10:[function(require,module,exports){
module.exports = Object.keys || require('./shim');


},{"./shim":13}],11:[function(require,module,exports){
...
},{"foreach":11,"is":12}],14:[function(require,module,exports){
(function (global){
"use strict"
...
(function (IDBService) {
    ...
    let _idbName;
    let _tables = [];
    function openDB(idbName = 'example', version = 1) {
        _idbName = idbName;
        let idbReq = indexedDB.open(idbName, version);
        idbReq.addEventListener('upgradeneeded', function (event) {
            _db = event.target.result;
        });
        idbReq.addEventListener('success', function (event) {
            _db = _db || event.target.result;
            let objStoreNameList = event.target.result.objectStoreNames;
            let i = 0;
    ...
    IDBService.openDB = openDB;
    function dropDB(idbName = _idbName) {
        indexedDB.deleteDatabase(idbName);
    }
    IDBService.dropDB = dropDB;
    function add(dataObj) {
        ;
    }
    IDBService.add = add;
    function getTransaction(tables = _tables, mode = 'readwrite') {
        return _db.transaction(tables, mode);
    }
    IDBService.getTransaction = getTransaction;
    function createObjectStore(table, keyPath, isAutoIncrement = true) {
        if (_tables.indexOf(table) === -1) {
            _tables.push(table);
        }
        return _db.createObjectStore(table, { keyPath: keyPath, autoIncrement: isAutoIncrement });
    }
    IDBService.createObjectStore = createObjectStore;
    function deleteObjectStore(tables) {
        // TODO
    }
    IDBService.deleteObjectStore = deleteObjectStore;
    function getTables() {
        return _tables;
    }
    IDBService.getTables = getTables;
})(IDBService = exports.IDBService || (exports.IDBService = {}));
;

},{}],18:[function(require,module,exports){
'use strict';
let osmAuth = require('osm-auth');
let authConfig = require('../../config/config.json');
require('es6-promise').polyfill();
require('leaflet.icon.glyph');
const indexeddb_class_1 = require('./indexeddb-class');
const coordinate_calc_1 = require('./coordinate-calc');
(function () {
    let idbService = indexeddb_class_1.IDBService;
    ...
    idbReq.addEventListener('upgradeneeded', function (event) {
        ...
        if (oldVersion < 1) {
            ...
            notesStore.createIndex('createdIdx', 'created', {
                'unique': false,
                'multiEntry': false,
            });
            notesStore.createIndex('modifiedIdx', 'modified', {
                'unique': false,
                'multiEntry': false,
            });
            let commentsStore = idbService.createObjectStore('comments', ['noteId', 'commentNum'], false);
            commentsStore.createIndex('noteIdIdx', 'noteId', {
                'unique': false,
                'multiEntry': false,
            });
        }
    ...
    idbReq.addEventListener('error', function (event) {
    ...
    document.addEventListener('DOMContentLoaded', function (event) {
        ...
        let getNotes = function (coordinate) {
            return new Promise(function (resolve, reject) {
                let edge = coordinate_calc_1.coordinateCalc.getCoordinateArea(coordinate, 10);
                auth.xhr({
                    method: 'GET',
        ...
        let setNotesList = function (notesXML) {
            ...
            for (let i = 0, notesCnt = notesXML.length; i < notesCnt; ++i) {
                ...
                for (let j = 0, commentCnt = commentList.length; j < commentCnt; ++j) {
                    ...
                    commentResult.addEventListener('success', function (event) {
                    });
                    commentResult.addEventListener('error', function (event) {
                        console.warn(event);
                    });
                ...
                notesResult.addEventListener('success', function (event) {
                });
                notesResult.addEventListener('error', function (event) {
        ...
        let displayNoteAndComment = function () {
            ...
            let noteTemplate = document.querySelector('#note-content');
            notesCursor.addEventListener('success', function (event) {
                if (notesCursor.result) {
                    let commentTemplate = document.querySelector('#comment-content');
                    let noteCursorVal = notesCursor.result;
                    ...
                    let created = noteTemplate.content.querySelector('.create-date > time');
                    created.dateTime = noteCursorVal.value.created.toISOString();
                    created.textContent = noteCursorVal.value.created.toLocaleString();
                    noteTemplate.content.querySelector('.modified-date').firstChild.textContent = 'メモ更新日：';
                    let modified = noteTemplate.content.querySelector('.modified-date > time');
                    modified.dateTime = noteCursorVal.value.modified.toISOString();
                    modified.textContent = noteCursorVal.value.modified.toLocaleString();
                    noteSection.querySelector('form').id = `${noteSection.id}-form`;
                    noteSection.querySelector(`#${noteSection.id}-form > select`).id
                        = noteSection.querySelector(`#${noteSection.id}-form > label:first-child`).htmlFor
                    ...
                    noteSection.querySelector(`#${noteSection.id}-form > select`).dataset.currentStatus = noteCursorVal.value.status;
                    noteSection.querySelector(`#${noteSection.id}-form > label > textarea`).id
                        = noteSection.querySelector(`#${noteSection.id}-form > label:last-child`).htmlFor
                    ...
                    submitButton.addEventListener('submit', (event) => {
                        postCommentAndMemoState(event, noteId).then(updateNoteCommentsList, (reject) => console.error(reject)).then(updateNotesDisplay, (reject) => console.error(reject));
                    ...
                    commentCursor.addEventListener('success', function (event) {
                        if (commentCursor.result === null) {
                            // コメントがなくなったら次のメモに移動
                            noteCursorVal.advance(1);
                        }
                        else {
                            let commCursorVal = commentCursor.result;
                            ...
                            let commentDate = comment.querySelector('time');
                            commentDate.dateTime = commCursorVal.value.date.toISOString();
                            commentDate.textContent = commCursorVal.value.date.toLocaleString();
                            ...
                            if (commCursorVal.value.user) {
                                let userOSMPage = comment.querySelector('span.user-osm-page > a');
                                userOSMPage.textContent = commCursorVal.value.user;
                                userOSMPage.href = commCursorVal.value.userURL;
                            }
                            else {
                                let userOSMPage = comment.querySelector('span.user-osm-page');
        ...
        let postCommentAndMemoState = function (event, noteId) {
            return new Promise((resolve, reject) => {
                let select = document.querySelector(`#note-${noteId}-changeNoteStatus`);
                let textarea = document.querySelector(`#note-${noteId}-addNoteComment`);
                let fixedEncodeURIComponent = (str) => encodeURIComponent(str).replace(/[!'()*]/g, (c) => '%' + c.codePointAt(0).toString(16));
                let params;
                if (select.value) {
                    switch (select.value) {
                        case 'closed':
                            ...
                            auth.xhr(params, (err, xhr) => {
                                if (err === null) {
                                    select.dataset.currentStatus = 'closed';
                        ...
                        case 'reopened':
                            ...
                            auth.xhr(params, (err, xhr) => {
                                if (err === null) {
                                    select.dataset.currentStatus = 'opened';
                    ...
                    auth.xhr(params, (err, responseXML) => {
                        if (err === null) {
        ...
        let updateNoteCommentsList = (xhrResult) => {
            return new Promise((resolve, reject) => {
                ...
                let response = xhrResult.response;
                let commentsIdx = commentsOS.index('noteIdIdx');
                ...
                noteCommentsRequest.onsuccess = (() => {
                    ...
                    for (let i = storedCommentsCount; i < lastCommentsCount; ++i) {
                        ...
                        commentResult.addEventListener('error', (event) => {
                            console.warn(event);
                            return reject(event);
                        });
                        if (commentDate > lastModified) {
                ...
                noteCommentsRequest.onerror = ((event) => {
                    console.warn(event);
                    return reject(event);
                });
                ...
                notesResult.addEventListener('success', (event) => {
                    return resolve({
        ...
        let updateNotesDisplay = (updateResult) => {
            ...
            let response = updateResult.responseXML;
            let lastModified = updateResult.lastModified;
            ...
            let noteModifiedElement = document.querySelector(`#note-${noteId} span.modified-date > time`);
            ...
            for (let i = displayedCommentCount; i < currentStoredCommentsCount; i++) {
                ...
                let commentDate = commentDOM.querySelector('time');
                ...
                if (comment.querySelector('user')) {
                    let userOSMPage = commentDOM.querySelector('span.user-osm-page > a');
                    ...
                    let userOSMPage = commentDOM.querySelector('span.user-osm-page');
                    userOSMPage.removeChild(userOSMPage.querySelector('a'));
                    userOSMPage.textContent = 'annonymous user';
                }
                document.querySelector(`#note-${noteId} > div > .comment-list`).appendChild(commentDOM);
            }
            document.querySelector(`#note-${noteId}-addNoteComment`).value = '';
        };
        let button = document.getElementById('get-near-notes');
        button.addEventListener('click', function () {
            if (auth.authenticated()) {
                getUserHomeLocation().then(getNotes, (error) => console.error(error)).then(setNotesList, (error) => console.error(error)).then(displayNoteAndComment, (error) => console.error(error));
            }
            else {
...
},{"../../config/config.json":1,"./coordinate-calc":16,"./indexeddb-class":17,"es6-promise":3,"leaflet.icon.glyph":4,"osm-auth":5}]},{},[18]);
